...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert any("exact write_permission_exceptions" in error for error in errors), errors
        ...
        evidence = [
            {"source": f"https://example.com/{index}", "finding": "fixture"} for index in range(4)
        ]
        ...
        errors = []
        checker.validate_workflows(checker.ROOT, errors, {})
        assert any("invalid contents permission None" in error for error in errors), errors

        workflow.unlink()
        errors = []
        checker.validate_workflows(checker.ROOT, errors, {})
        assert any("must contain at least one workflow" in error for error in errors), errors
        ...
        errors = []
        checker.validate_workflow_policy({"github_actions": base_workflow_policy}, errors)
