...
def validate_policy_shape(contract: dict[str, Any], errors: list[str]) -> dict[str, Any] | None:
    ...
    if not isinstance(roles, list) or set(roles) != REQUIRED_STRUCTURAL_ROLES:
        errors.append(f"{hierarchy_prefix}.structural_role_exclusions has the wrong set")
    ...
    for path in sorted((*workflows.glob("*.yml"), *workflows.glob("*.yaml"))):
        ...
        for index, line in enumerate(lines):
            ...
            if not FULL_SHA_PATTERN.fullmatch(ref):
                errors.append(f"workflow action is not SHA-pinned: {rel}:{index + 1}: {action}")
            if CHECKOUT_PATTERN.search(action) and not checkout_is_nonpersistent(lines, index):
                errors.append(
