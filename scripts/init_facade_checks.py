...
def _optional_import_try(node: ast.Try) -> bool:
    body_is_facade = all(
        ...
        or (isinstance(item, (ast.Assign, ast.AnnAssign)) and _static_metadata_assignment(item))
    ...
    return not node.orelse and not node.finalbody and body_is_facade and handlers_are_fallbacks
