...
PERMISSION_SCOPE_PATTERN = re.compile(r"^\s*([a-z][a-z0-9-]*)\s*:\s*['\"]?write['\"]?\s*(?:#.*)?$")
...
def _validate_exception_metadata(item: dict[str, Any], prefix: str, errors: list[str]) -> None:
...
def _validate_action_references(rel: str, lines: list[str], errors: list[str]) -> None:
    for index, line in enumerate(lines):
        ...
        if CHECKOUT_PATTERN.search(action) and not checkout_is_nonpersistent(lines, index):
            errors.append(
                f"checkout credentials persist without an exact policy exception: {rel}:{index + 1}"
            )
