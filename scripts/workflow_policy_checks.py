...
def validate_workflow_policy(contract: dict[str, Any], errors: list[str]) -> dict[str, set[str]]:
    ...
    for item in raw_exceptions:
        if not isinstance(item, dict) or not REQUIRED_EXCEPTION_FIELDS.issubset(item):
            errors.append("write permission exception missing path/scopes/reason/owner/review_by")
            continue
        path = str(item["path"])
        scopes = item["scopes"]
        ...
        if (
            ...
            errors.append(f"write permission exception {path}: scopes must be non-empty")
            continue
