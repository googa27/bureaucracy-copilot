...
def validate_workflow_policy(contract: dict[str, Any], errors: list[str]) -> dict[str, set[str]]:
    ...
    for index, item in enumerate(raw_exceptions):
        ...
        if (
            ...
            errors.append(f"{label} ({path}): scopes must be non-empty lowercase strings")
            continue
...
def _permission_write_scopes(value: Any, label: str, errors: list[str]) -> set[str]:
    ...
    for scope, raw_access in value.items():
        ...
        if scope_name != scope_name.lower():
            errors.append(f"{label}: permission scope must be lowercase: {scope_name!r}")
            continue
    ...
    for job_name, raw_job in jobs.items():
        ...
        for index, step in enumerate(steps):
            if not isinstance(step, dict):
                errors.append(f"{label}: job {job_name!r} step {index} must be a mapping")
                continue
            uses = step.get("uses")
            if uses is None:
                continue
            if isinstance(uses, str):
                ...
                errors.append(f"{label}: job {job_name!r} step {index} uses must be a string")
...
def _validate_local_action(
    ...
    if action_path is None:
        errors.append(f"{label}: local action metadata is missing or escapes root: {uses}")
        return
    ...
    for index, step in enumerate(steps):
        ...
        if not isinstance(nested, str):
            errors.append(f"{action_path}: composite step {index} uses must be a string")
            continue
...
def _validate_action_reference(
    ...
    if action != "actions/checkout":
        return
    if step is None:
        errors.append(f"{label}: actions/checkout is invalid as a job-level reusable workflow")
        return
...
def validate_workflows(
    ...
    for path in workflow_paths:
        ...
        for uses, step in _uses_entries(document, rel, errors):
            _validate_action_reference(root, uses, step, rel, errors, visited_local_actions)
